...
#Monkeypatch unichr so as not to crash and burn on narrow Python builds
real_unichr = unichr
def unichr(arg):
    if arg > sys.maxunicode:
        if 0xfffd > sys.maxunicode:
            return real_unichr(ord('?'))
        return real_unichr(0xfffd)
    return real_unichr(arg)
...
def main():
    ...
    p.add_option("--escape-all", action="store_true", dest="escape_snob",
                 default=False, help="Escape all special characters.  Output is less readable, \
                                      but avoids corner case formatting issues.")
